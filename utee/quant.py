...
class Quant_F(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input, delta_x, quant_method, bits, overflow_rate):
        # ctx is a context object that can be used to stash information for backward computation
        if quant_method == 'linear':
            sf = bits - 1. - compute_integral_part(input, overflow_rate=overflow_rate)
            output = linear_quantize(input, sf, bits=bits)
        elif quant_method == 'log':
            output = log_minmax_quantize(input, bits=bits)
        elif quant_method == 'minmax':
            output = min_max_quantize(input, bits=bits)
        else:
            output = tanh_quantize(input, bits=bits)
        return output
